...
class _DeprecationTestCase:
    ...
    def assert_deprecated(self, function, log, num=1, ignore_others=False,
    ...
    def assert_not_deprecated(self, function, log, args=(), kwargs={}):
        """Test that warnings are not raised.

        This is just a shorthand for:

        self.assert_deprecated(function, log, num=0, ignore_others=True,
...
# need to rewrite this to do testing for new fixture technique
# class TestTestDeprecated:
#     def test_assert_deprecated(self, log):
...
#         test_case_instance.assert_deprecated(foo, log)
#         #test_case_instance.teardown_method()

@pytest.mark.usefixtures("log")
@pytest.mark.thread_unsafe("warnings is threads_unsafe?")
class TestBincount(_DeprecationTestCase):
    # 2024-07-29, 2.1.0
    @pytest.mark.parametrize('badlist', [[0.5, 1.2, 1.5],
                                         ['0', '1', '1']])
    def test_bincount_bad_list(self, badlist, log):
        self.assert_deprecated(lambda: np.bincount(badlist), log)
...
class TestGeneratorSum(_DeprecationTestCase):
    # 2018-02-25, 1.15.0
    def test_generator_sum(self, log):
        self.assert_deprecated(np.sum, log, args=((i for i in range(5)),))
...
class BuiltInRoundComplexDType(_DeprecationTestCase):
    ...
    def test_deprecated(self, log):
        for scalar_type in self.deprecated_types:
            scalar = scalar_type(0)
            self.assert_deprecated(round, log, args=(scalar,))
            self.assert_deprecated(round, log, args=(scalar, 0))
            self.assert_deprecated(round, log, args=(scalar,), kwargs={'ndigits': 0})

    def test_not_deprecated(self, log):
        for scalar_type in self.not_deprecated_types:
            scalar = scalar_type(0)
            self.assert_not_deprecated(round, log, args=(scalar,))
            self.assert_not_deprecated(round, log, args=(scalar, 0))
            self.assert_not_deprecated(round, log, args=(scalar,), kwargs={'ndigits': 0})
...
class FlatteningConcatenateUnsafeCast(_DeprecationTestCase):
    ...
    def test_deprecated(self, log):
        self.assert_deprecated(np.concatenate, log,
                args=(([0.], [1.]),),
                kwargs={'axis': None, 'out': np.empty(2, dtype=np.int64)})

    def test_not_deprecated(self, log):
        self.assert_not_deprecated(np.concatenate, log,
                args=(([0.], [1.]),),
                kwargs={'axis': None, 'out': np.empty(2, dtype=np.int64),
                        'casting': "unsafe"})
...
class TestCtypesGetter(_DeprecationTestCase):
    ...
    def test_deprecated(self, name: str, log) -> None:
        func = getattr(self.ctypes, name)
        self.assert_deprecated(func, log)

    @pytest.mark.parametrize(
        "name", ["data", "shape", "strides", "_as_parameter_"]
    )
    def test_not_deprecated(self, name: str, log) -> None:
        self.assert_not_deprecated(lambda: getattr(self.ctypes, name), log)
...
class TestMachAr(_DeprecationTestCase):
    ...
    def test_deprecated_module(self, log):
        self.assert_deprecated(lambda: np._core.MachAr, log)
...
class TestQuantileInterpolationDeprecation(_DeprecationTestCase):
    ...
    def test_deprecated(self, func, log):
        self.assert_deprecated(
            lambda: func([0., 1.], 0., interpolation="linear"), log)
        self.assert_deprecated(
            lambda: func([0., 1.], 0., interpolation="nearest"), log)
...
class TestScalarConversion(_DeprecationTestCase):
    # 2023-01-02, 1.25.0
    def test_float_conversion(self, log):
        self.assert_deprecated(float, log, args=(np.array([3.14]),))
...
class TestPyIntConversion(_DeprecationTestCase):
    ...
    def test_deprecated_scalar(self, dtype, log):
        ...
        for creation_func in [scalar, assign, create]:
            try:
                self.assert_deprecated(
                        lambda: creation_func(info.min - 1, dtype), log)
            except OverflowError:
                pass  # OverflowErrors always happened also before and are OK.

            try:
                self.assert_deprecated(
                        lambda: creation_func(info.max + 1, dtype), log)
            except OverflowError:
                pass  # OverflowErrors always happened also before and are OK.
...
class TestDeprecatedFinfo(_DeprecationTestCase):
    ...
    def test_deprecated_none(self, log):
        self.assert_deprecated(np.finfo, log, args=(None,))


@pytest.mark.usefixtures("log")
@pytest.mark.thread_unsafe("warnings is threads_unsafe?")
class TestMathAlias(_DeprecationTestCase):
    def test_deprecated_np_lib_math(self, log):
        self.assert_deprecated(lambda: np.lib.math, log)
...
class TestLibImports(_DeprecationTestCase):
    ...
    def test_lib_functions_deprecation_call(self, log):
        ...
        self.assert_deprecated(lambda: safe_eval("None"), log)

        data_gen = lambda: TextIO('A,B\n0,1\n2,3')
        kwargs = {'delimiter': ",", 'missing_values': "N/A", 'names': True}
        self.assert_deprecated(lambda: recfromcsv(data_gen()), log)
        self.assert_deprecated(lambda: recfromtxt(data_gen(), **kwargs), log)

        self.assert_deprecated(lambda: disp("test"), log)
        self.assert_deprecated(get_array_wrap, log)
        self.assert_deprecated(lambda: maximum_sctype(int), log)

        self.assert_deprecated(lambda: in1d([1], [1]), log)
        self.assert_deprecated(lambda: row_stack([[]]), log)
        self.assert_deprecated(lambda: trapz([1], [1]), log)
        self.assert_deprecated(lambda: np.chararray, log)
...
class TestDeprecatedDTypeAliases(_DeprecationTestCase):
    ...
    def test_a_dtype_alias(self, log):
        for dtype in ["a", "a10"]:
            f = lambda: np.dtype(dtype)
            self._check_for_warning(f)
            self.assert_deprecated(f, log)
...
class TestDeprecatedArrayWrap(_DeprecationTestCase):
    ...
    def test_deprecated(self, log):
        ...
        test1 = Test1()
        test2 = Test2()
        self.assert_deprecated(lambda: np.negative(test1), log)
        assert test1.called
        self.assert_deprecated(lambda: np.negative(test2), log)
        assert test2.called
...
class TestDeprecatedArrayAttributeSetting(_DeprecationTestCase):
    ...
    def test_deprecated_strides_set(self, log):
        x = np.eye(2)
        self.assert_deprecated(setattr, log, args=(x, 'strides', x.strides))
...
class TestDeprecatedDTypeParenthesizedRepeatCount(_DeprecationTestCase):
    ...
    def test_parenthesized_repeat_count(self, string, log):
        self.assert_deprecated(np.dtype, log, args=(string,))
...
class TestDeprecatedSaveFixImports(_DeprecationTestCase):
    ...
    def test_deprecated(self, log):
        with temppath(suffix='.npy') as path:
            ...
            self.assert_not_deprecated(np.save, log, args=sample_args)
            self.assert_deprecated(np.save, log, args=sample_args,
...
class TestDTypeAlignBool(_VisibleDeprecationTestCase):
    ...
    def test_deprecated(self, log):
        ...
        # a tuple).
        self.assert_deprecated(lambda: np.dtype("f8", align=3), log)

    @pytest.mark.parametrize("align", [True, False, np.True_, np.False_])
    def test_not_deprecated(self, align, log):
        # if the user passes a bool, it is accepted.
        self.assert_not_deprecated(lambda: np.dtype("f8", align=align), log)
...
class TestFlatiterIndexing0dBoolIndex(_DeprecationTestCase):
    ...
    def test_0d_boolean_index_deprecated(self, log):
        arr = np.arange(3)
        # 0d boolean indices on flat iterators are deprecated
        self.assert_deprecated(lambda: arr.flat[True], log)

    def test_0d_boolean_assign_index_deprecated(self, log):
        arr = np.arange(3)

        def assign_to_index():
            arr.flat[True] = 10

        self.assert_deprecated(assign_to_index, log)
...
class TestFlatiterIndexingFloatIndex(_DeprecationTestCase):
    ...
    def test_float_index_deprecated(self, log):
        arr = np.arange(3)
        # float indices on flat iterators are deprecated
        self.assert_deprecated(lambda: arr.flat[[1.]], log)

    def test_float_assign_index_deprecated(self, log):
        arr = np.arange(3)
